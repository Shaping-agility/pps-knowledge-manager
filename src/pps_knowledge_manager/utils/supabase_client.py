...
def get_supabase_client(database_name: str | None = None) -> Client:
    if not SUPABASE_URL or not SUPABASE_KEY:
        raise ValueError("Supabase credentials are not set in environment variables.")

    # For now, we'll use the default Supabase client
    # TODO: Implement database-specific connection when needed
